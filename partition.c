...
void part_add_size(XboxPartitionTable *PartTbl, int PartNumber, ULONG TotalSectors, ULONG addition)
{
	...
	PartTbl->TableEntries[PartNumber].LBASize = safeadd (PartTbl->TableEntries[PartNumber].LBASize,addition);
}
...
void part_change_size(XboxPartitionTable *PartTbl, int PartitionNum, ULONG TotalSectors, int TrigValue, int change_start, int Direction, bool fine)
{
	int num_sectors;
...
u_int32_t part_get_current_free_below(XboxPartitionTable *PartTbl, int PartNumber)
{
	int i;
	ULONG starts[14], ends[14];
...
void part_setup_desiredsize_partitions(XboxPartitionTable *PartTbl, XboxPartitionTable *BackupPartTbl, ULONG TotalSectors,int FdesiredPercent,u_int64_t tb_totalsectors)
{
	...
	PartTbl->TableEntries[5].LBAStart = XBOX_PART6_LBA_START;
		// F gets desired size
		if (tb_totalsectors==0)
			PartTbl->TableEntries[5].LBASize = (ULONG) (TotalSectors - PartTbl->TableEntries[5].LBAStart) * (float)(FdesiredPercent/100.0);
		else
				PartTbl->TableEntries[5].LBASize = safesubstract (tb_totalsectors,PartTbl->TableEntries[5].LBAStart) * (float)(FdesiredPercent/100.0);
		...
		if (tb_totalsectors==0)
				...
				PartTbl->TableEntries[6].LBASize = safesubstract (tb_totalsectors,PartTbl->TableEntries[6].LBAStart);
...
void part_setup_standard_partitions(XboxPartitionTable *PartTbl, XboxPartitionTable *BackupPartTbl, ULONG TotalSectors, int Type,u_int64_t tb_totalsectors)
{
	...
	PartTbl->TableEntries[5].Flags |= PE_PARTFLAGS_IN_USE;
	PartTbl->TableEntries[5].LBAStart = XBOX_PART6_LBA_START;
	if (TotalSectors>= LBASIZE_ONE_TERA && TotalSectors <= LBASIZE_TWO_TERA)
	{
	...
	case 0:
		if (TotalSectors & 0xf0000000UL)
			...
			if (tb_totalsectors==0)
				...
				PartTbl->TableEntries[5].LBASize = safesubstract (tb_totalsectors,PartTbl->TableEntries[5].LBAStart);
			PartTbl->TableEntries[6].Flags &= ~PE_PARTFLAGS_IN_USE;
		break;
	case 1:
		// F gets all
...
ULONG safesubstract(u_int64_t a,ULONG b)
{
        ...
        u_int64_t result;
        result=a-tempb;
        if ( result > LBASIZE_MAX)
                return LBASIZE_MAX;
        return (ULONG)(result>>32);
